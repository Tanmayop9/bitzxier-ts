    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.permissions.has('Administrator')) {
            return message.channel.send({
                embeds: [
                    client.util.embed()
                        .setColor(client.color)
                        .setDescription(`You must have \`Administrator\` perms to run this command.`)
                ]
            });
        }
        ...
        if (action === 'reset') {
            ...
            if (roles.length === 0)
                ...
                response = await removeAutoRole(message, roles[0], roleType);
        }
        else if (action === 'list') {
...
async function addAutoRole({ guild, client }, role, type) {
    ...
    if (settings[roleType].length === maxRoles)
        ...
        settings[roleType].push(role.id);
    await settings.save();
...
async function removeAutoRole({ guild, client }, role, type) {
    ...
    if (settings[roleType].length === 0)
        ...
        settings[roleType] = settings[roleType].filter((r) => r !== role.id);
    await settings.save();
