    ...
    run: async (client, message, args) => {
        ...
        if (args[0].toLowerCase() === 'panel') {
            ...
            if (args[1].toLowerCase() === 'setup') {
                ...
                const channel = await promptUser(`Please provide the ticket creation channel.`, (response) => {
                ...
                if (!channel)
                    return message.channel.send({ embeds: [new EmbedBuilder().setColor(client.color).setDescription(`${client.emoji.cross} | Failed to set up ticket panel.`)] });
                ticketSetup.channelId = channel;
                // Prompt for ticket creation category
                const category = await promptUser(`Please provide the ticket creation category.`, (response) => {
                ...
                const logsDecision = await promptUser(`Do you need a ticket logs channel? Respond with \`yes\` or \`no\`.`, (response) => ['yes', 'no'].includes(response.content.toLowerCase()) ? response.content.toLowerCase() : null);
                if (!logsDecision)
                    return message.channel.send({ embeds: [new EmbedBuilder().setColor(client.color).setDescription(`${client.emoji.cross} | Failed to set up ticket panel.`)] });
                if (logsDecision === 'yes') {
                    const logsChannel = await promptUser(`Please provide the ticket logs channel.`, (response) => {
                        const channel = response.mentions.channels.first() || message.guild.channels.cache.get(response.content);
                        return channel && channel.type === ChannelType.GuildText ? channel.id : null;
                    });
                    if (!logsChannel)
                ...
                const transcriptDecision = await promptUser(`Do you need a transcript channel? Respond with \`yes\` or \`no\`.`, (response) => ['yes', 'no'].includes(response.content.toLowerCase()) ? response.content.toLowerCase() : null);
                if (!transcriptDecision)
                    return message.channel.send({ embeds: [new EmbedBuilder().setColor(client.color).setDescription(`${client.emoji.cross} | Failed to set up ticket panel.`)] });
                if (transcriptDecision === 'yes') {
                    const transcriptChannel = await promptUser(`Please provide the transcript channel.`, (response) => {
                        const channel = response.mentions.channels.first() || message.guild.channels.cache.get(response.content);
                        return channel && channel.type === ChannelType.GuildText ? channel.id : null;
                    });
                ...
                const supportRoleDecision = await promptUser(`Do you want to set a support role? Respond with \`yes\` or \`no\`.`, (response) => ['yes', 'no'].includes(response.content.toLowerCase()) ? response.content.toLowerCase() : null);
                if (!supportRoleDecision)
                    return message.channel.send({ embeds: [new EmbedBuilder().setColor(client.color).setDescription(`${client.emoji.cross} | Failed to set up ticket panel.`)] });
                if (supportRoleDecision === 'yes') {
                    const supportRole = await promptUser(`Please provide the support role.`, (response) => {
                ...
                const staffRoleDecision = await promptUser(`Do you want to set a staff role? Respond with \`yes\` or \`no\`.`, (response) => ['yes', 'no'].includes(response.content.toLowerCase()) ? response.content.toLowerCase() : null);
                if (!staffRoleDecision)
                    return message.channel.send({ embeds: [new EmbedBuilder().setColor(client.color).setDescription(`${client.emoji.cross} | Failed to set up ticket panel.`)] });
                if (staffRoleDecision === 'yes') {
                    const staffRole = await promptUser(`Please provide the staff role.`, (response) => {
                        const role = response.mentions.roles.first() || message.guild.roles.cache.get(response.content);
                        return role ? role.id : null;
                    });
                    if (!staffRole)
            ...
            if (args[1].toLowerCase() === 'rename') {
                ...
                try {
                    ...
                    if (args[2]) {
                        let channel = await getChannelFromMention(message, args[2]) ||
