...
export default {
    ...
    run: async (client, message, args) => {
        ...
        if (!data) {
            ...
            if (data.channel)
                logs.addFields({ name: 'Channel logs', value: `<#${data?.channel}>` });
            if (!data.channel)
                logs.addFields({ name: 'Channel logs', value: `\`NOT SET\`` });
            if (data.rolelog)
                logs.addFields({ name: 'Role logs', value: `<#${data?.rolelog}>` });
            if (!data.rolelog)
                logs.addFields({ name: 'Role logs', value: `\`NOT SET\`` });
            if (data.memberlog)
                logs.addFields({ name: 'Member logs', value: `<#${data?.memberlog}>` });
            if (!data.memberlog)
                logs.addFields({ name: 'Member logs', value: `\`NOT SET\`` });
            if (data.modlog)
                logs.addFields({ name: 'Mod logs', value: `<#${data?.modlog}>` });
            if (!data.modlog)
                logs.addFields({ name: 'Mod logs', value: `\`NOT SET\`` });
            if (data.message)
                logs.addFields({ name: 'Message logs', value: `<#${data?.message}>` });
            if (!data.message)
                logs.addFields({ name: 'Message logs', value: `\`NOT SET\`` });
            if (data.voice)
                logs.addFields({ name: 'Voice logs', value: `<#${data?.voice}>` });
            if (!data.voice)
                logs.addFields({ name: 'Voice logs', value: `\`NOT SET\`` })
                    .setTimestamp();
