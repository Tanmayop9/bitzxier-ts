                ...
                await message.channel.send({
                    embeds: [
                        new EmbedBuilder()
                            ...
                            .addFields({ name: 'Types of Logging Enabled', value: '\`Message (Update, Delete)\`\n\`Member (Join, Leave, Role Update)\`\n\`Role (Create, Update, Delete)\`\n\`Channel (Create, Update, Delete)\`\n\`Modlog (Ban, Unban, Kick)\`\n \`Voice Logs (Member join,leave,move)\`' })
                            .addFields({ name: 'What happens now?', value: 'The bot will log various events in the configured channel, including messages, member actions, role changes, server updates, and channel modifications.' })
                    ],
                });
            }
        }
...
//# sourceMappingURL=logall.js.map