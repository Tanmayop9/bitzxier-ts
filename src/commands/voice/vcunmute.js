        ...
        if (!member1) {
            return message.channel.send({
                embeds: [
                    client.util.embed()
                        .setColor(client.color)
                        .setDescription(`You must mention someone whom you want to unmute in your vc.`)
                ]
            });
        }
        ...
        try {
            ...
            message.channel.send({
                embeds: [
                    client.util.embed()
                        .setColor(client.color)
                        .setDescription(`${client.emoji.tick} | Successfully UnMuted <@${member.user.id}> From Voice!`)
                ]
            });
        }
        catch (err) {
            return message.channel.send({
                embeds: [
                    client.util.embed()
                        .setColor(client.color)
                        .setDescription(`I was unable to voice unmute <@${member.user.id}>.`)
                ]
            });
        }
    }
};
//# sourceMappingURL=vcunmute.js.map