        ...
        if (!member1) {
            return message.channel.send({
                embeds: [
                    client.util.embed()
                        .setColor(client.color)
                        .setDescription(`You must mention someone whom you want to deafen in your vc.`)
                ]
            });
        }
        ...
        try {
            ...
            message.channel.send({
                embeds: [
                    client.util.embed()
                        .setColor(client.color)
                        .setDescription(`${client.emoji.tick} | Successfully deafened <@${member.user.id}> From Voice!`)
                ]
            });
        }
        catch (err) {
            return message.channel.send({
                embeds: [
                    client.util.embed()
                        .setColor(client.color)
                        .setDescription(`I was unable to voice deafen <@${member.user.id}>.`)
                ]
            });
        }
    }
};
//# sourceMappingURL=vcdeafen.js.map