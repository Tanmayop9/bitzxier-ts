    ...
    run: async (client, message, args) => {
        ...
        if (!own &&
            ...
            const higherole = client.util.embed()
                .setColor(client.color)
                .setDescription(`${client.emoji.cross} | Only Server Owner Or Extraowner Having Higher Role Than Me Can Run This Command`);
        ...
        if (option === 'enable') {
            ...
            const manageableRoles = message.guild.roles.cache.filter((role) => role.comparePositionTo(botHighestRole) < 0 &&
                role.name !== '@everyone' &&
            ...
            const promises = manageableRoles.map(async (role) => {
                ...
                const permissionsBitfield = new PermissionsBitField(adminPermissions).bitfield;
                if (adminPermissions.length > 0) {
                    ...
                    await role
                        .setPermissions(permissions, `BITZXIER NIGHTMODE ENABLED`)
                        .catch((err) => {
                ...
                if (serverData) {
                    ...
                    await rolePermissionSchema
                        .findOneAndUpdate({ guildId: message.guild.id, roleId: role.id }, { adminPermissions: permissionsBitfield }, { upsert: true })
                        .catch((err) => null);
                }
                else {
                    await client.util.sleep(3000);
            ...
            return message.channel.send({ content: 'Nightmode enabled! Dangerous Permissions Disabled For Manageable Roles.'
            });
        }
