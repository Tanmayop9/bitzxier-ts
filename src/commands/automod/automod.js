                ...
                if (action === 'add') {
                            ...
                            return await message.channel.send({
                                embeds: [
                                    client.util.embed()
                                        ...
                                        .setTitle('AUTOMOD WHITELIST ROLE OPTIONS')
                                        .addFields([
                                        {
                                            name: `\`Whitelist Role\``,
                                            value: `**Shows the current page.**`
                    ...
                    if (index === -1) {
                        return message.channel.send({
                            embeds: [
                                client.util.embed()
                                    .setColor(client.color)
                                    .setDescription(`${client.emoji.cross} | The ${itemName} is not in ${type}'s whitelist`)
                            ]
                        });
                    }
                    ...
                    message.channel.send({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.tick} | Successfully removed ${itemName} ID (${idToRemove}) from ${type}'s whitelist.`)
                        ]
                    });
                }
                    ...
                    buttonCollector.on('collect', async (interaction) => {
                        if (interaction.user.id !== message.author.id)
                            return interaction.editReply({ content: `You are not allowed to interact with these buttons.`, ephemeral: true });
                        ...
                        let i = 0;
                        const membersList = entries.map(entry => type === 'user' ? `${++i}. <@${entry.id}> | (${entry.id})` :
