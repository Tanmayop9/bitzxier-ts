...
export default {
    ...
    run: async (client, message, args) => {
        ...
        const option = args[0];
        const isActivatedAlready = (await client.db.get(`antiswear_${message.guild.id}`)) ?? null;
        const antiswear = client.util.embed()
            .setThumbnail(client.user.avatarURL({ dynamic: true }))
            .setColor(client.color)
            .setTitle('__**AntiSwear**__')
            .setDescription("Fortify your server's security with Antiswear! Our cutting-edge algorithms swiftly detect and neutralize profanity, ensuring a safe and respectful environment for your community.")
            .addFields({
            ...
            case 'enable':
                if (!isActivatedAlready) {
                    ...
                    const antilinkEnableMessage = client.util.embed()
                        ...
                        .setDescription('**Congratulations! AntiSwear has been successfully enabled on your server.**')
                        .addFields({
                    ...
                    const antilinkSettingsEmbed = client.util.embed()
                        ...
                        .addFields({ name: 'Current Status',
                        value: `AntiSwear is already enabled on your server.\n\nCurrent Status: ${enable}`
                    })
                        .addFields({ name: 'To Disable',
                        value: `To disable AntiSwear, use \`${prefix}antiswear disable\``
                    })
                        .setTimestamp()
            ...
            case 'word':
                if (args[1] === 'add') {
                    if (!args[2]) {
                        return message.channel.send({
                            embeds: [
                                client.util.embed()
                                    .setColor(client.color)
                                    .setDescription(`${client.emoji.cross} | Oops! Please provide a valid word for my badwords list.`)
                            ]
                        });
                    }
                    await client.db
                        ...
                        if (!data) {
                            await client.db.set(`antiswearwords_${message.guild.id}`, []);
                            ...
                            const saiop = client.util.embed()
                                .setColor(client.color)
                                .setDescription(`Great news! Your server is now configured perfectly. Feel free to use AntiSwear module and enjoy using your server hassle-free! 🚀`);
                        ...
                        if (word.length > 30) {
                            return message.channel.send({
                                embeds: [
                                    client.util.embed()
                                        .setColor(client.color)
                                        .setDescription(`${client.emoji.cross} | The word "${word}" has more than 15 characters and cannot be added to the bad words list.`)
                                ]
                            });
                        }
                        if (word) {
                            if (!data.includes(word)) {
                                await client.db.push(`antiswearwords_${message.guild.id}`, word);
                                return message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                                            .setDescription(`${client.emoji.tick} | Success! The \`${word}\` word has been successfully added to bad words list.`)
                                    ]
                                });
                            }
                            else {
                                return message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                                            .setDescription(`${client.emoji.cross} | Oops! It appears that \`${word}\` is already in my bad words list.`)
                                    ]
                                });
                            }
                        }
                    ...
                    if (!args[2]) {
                        return message.channel.send({
                            embeds: [
                                client.util.embed()
                                    .setColor(client.color)
                                    .setDescription(`${client.emoji.cross} | Oops! Please provide a valid word for removing in my badwords list.`)
                            ]
                        });
                    }
                    await client.db
                        ...
                        if (word) {
                            if (data.includes(word)) {
                                await client.db.pull(`antiswearwords_${message.guild.id}`, word);
                                return message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                                            .setDescription(`${client.emoji.tick} | Success! The \`${word}\` word has been successfully removed from bad words list.`)
                                    ]
                                });
                            }
                            else {
                                return message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                                            .setDescription(`${client.emoji.cross} | Oops! It appears that \`${word}\` is not in my bad words list.`)
                                    ]
                                });
                            }
                        }
                    ...
                    await client.db
                        ...
                        if (data.length > 0) {
                            const existingData = await client.db.get(`antiswearwords_${message.guild.id}`);
                            if (existingData) {
                                await client.db.set(`antiswearwords_${message.guild.id}`, []);
                                return message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                                            .setDescription(`✨ Success! The server's bad word list has been cleared. You're all set to customize and optimize your experience now.`)
                                    ]
                                });
                            }
                        }
                            ...
                            return message.channel.send({
                                embeds: [
                                    client.util.embed()
                                        .setColor(client.color)
                                        .setDescription(`🌐 Whoops! It looks like no bad words has been set up in this server yet. No worries, though! You can easily configure one to tailor your experience and enhance server functionality.`)
                                ]
                            });
                        }
                    });
                    ...
                    if (Array.isArray(words) && words.length !== 0) {
                        words.forEach((word) => mentions.push(`\`${i++}\`   \`${word}\``));
                        ...
                        return message.channel.send({
                            embeds: [
                                client.util.embed()
                                    .setColor(client.color)
                                    .setDescription(`${client.emoji.cross} | Oops! It seems like there's no designated badwords set up in this server. No worries, though! You can easily configure one to enhance your experience.`)
                            ]
                        });
                    }
                }
            ...
            case 'punishment':
                ...
                if (punishment === 'mute') {
                    ...
                    const embedMessage = client.util.embed()
                        ...
                        .setDescription('The AntiSwear punishment has been successfully configured.')
...
//# sourceMappingURL=antiswear.js.map