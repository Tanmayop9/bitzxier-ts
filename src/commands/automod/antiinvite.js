export default {
    ...
    run: async (client, message, args) => {
        ...
        const option = args[0];
        const isActivatedAlready = (await client.db.get(`antiinvite_${message.guild.id}`)) || { enabled: null, punishment: null };
        const antiinvite = client.util.embed()
            .setThumbnail(client.user.avatarURL({ dynamic: true }))
            .setColor(client.color)
            .setTitle('__**Antiinvite**__')
            .setDescription("Protect your server with AntiInvite! Our intelligent system detects and blocks Discord invite links, ensuring your community remains safe from spam and unauthorized promotions.")
            .addFields([
            {
            ...
            case 'enable':
                if (!isActivatedAlready.enabled) {
                    ...
                    const antiinviteEnableMessage = client.util.embed()
                        ...
                        .setDescription('**Congratulations! Antiinvite has been successfully enabled on your server.**')
                        .addFields({
                    ...
                    const antiinviteSettingsEmbed = client.util.embed()
                        .setTitle(`Antiinvite Settings for ${message.guild.name} ${protect}`)
                        .setColor(client.color)
                        .setDescription('**Antiinvite is already enabled on your server.**')
                        .addFields({
                        name: 'Current Status',
            ...
            case 'disable':
                if (isActivatedAlready.enabled) {
                    ...
                    const antiinviteDisableMessage = client.util.embed()
                        ...
                        .setDescription('**Antiinvite has been successfully disabled on your server.**')
                        .addFields({
                        name: 'Impact',
            ...
            case 'punishment':
                ...
                if (punishment === 'mute') {
                    ...
                    const embedMessage = client.util.embed()
                        ...
                        .setDescription('The antiinvite punishment has been successfully configured.')
