    ...
    run: async (client, message, args) => {
        ...
        if (member && !['list', 'remove', 'reset'].includes(args[0].toLowerCase())) {
            ...
            if (otherSubcommand === 'reset') {
                ...
                const warnList = warnings.map(warn => `**ID:** \`${warn.warnId}\` | Moderator: <@${warn.moderatorId}>, Reason: \`${warn.reason}\`, Timestamp: \`${warn.timestamp}\``).join('\n');
                return message.channel.send({
                    embeds: [
                        client.util.embed()
                            .setColor(client.color)
