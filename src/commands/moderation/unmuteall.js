    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.permissions.has('ModerateMembers')) {
            let error = client.util.embed()
                ...
                .setDescription(`You must have \`Moderate Members\` permission to use this command.`);
            return message.channel.send({ embeds: [error] });
        }
        if (!message.guild.members.me.permissions.has('ModerateMembers')) {
            let error = client.util.embed()
                .setColor(client.color)
                .setDescription(`I must have \`Moderate Members\` permission to use this command.`);
            return message.channel.send({ embeds: [error] });
        }
        ...
        if (!message.guild.Unmuteall) {
            ...
            interactionMessage.edit({
                embeds: [
                    embed.setDescription(`**Are you sure you want to unmuteall all muted members in this server?**`),
                ], components: [row],
            });
            ...
            collector.on('collect', async (interaction) => {
                if (interaction.isButton()) {
                    if (interaction.customId === 'yes_button') {
                        interactionMessage.edit({
                            embeds: [
                                embed.setDescription(`${client.emoji.tick} | Successfully started unmuting to all muted members.`),
                            ],
                            components: [],
                        });
                        collector.stop();
            ...
            return message.channel.send({
                embeds: [
                    embed.setDescription(`${client.emoji.cross} | There is already a Unmute all process is going on in the server.`)
                        .setColor(client.color),
                ],
            });
        }
