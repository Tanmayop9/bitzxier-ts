        ...
        try {
                ...
                return message.channel.send({
                    embeds: [
                        client.util.embed()
                            ...
                            .setDescription(`${client.emoji.cross} | You need \`MANAGE SERVER\` permissions to use this command.`)
                    ]
                });
            }
            if (!message.guild.members.me.permissions.has('Administrator')) {
                return message.channel.send({
                    embeds: [
                        client.util.embed()
                            .setColor(client.color)
                            .setDescription(`${client.emoji.cross} | I require \`ADMINISTRATOR\` permissions to execute this command.`)
                    ]
                });
            }
            ...
            if (!category || !['channel', 'whitelist'].includes(category)) {
                const infoEmbed = new EmbedBuilder()
                    ...
                    .setDescription(`The \`media\` command allows you to configure media-only channels and manage whitelisted roles or users.\n\nUse the following subcommands to set up and manage media configurations.`)
                    .addFields([
                ...
                if (!action || !['add', 'remove', 'reset', 'list'].includes(action)) {
                    return message.channel.send({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.cross} | Invalid action! Use \`add\`, \`remove\`, \`reset\`, or \`list\`.`)
                        ]
                    });
                }
                if (action === 'add')
                    return handleAdd('channel');
                if (action === 'remove')
                    return handleRemove('channel');
                if (action === 'reset')
                    return handleReset('channel');
                if (action === 'list')
            ...
            if (category === 'whitelist') {
                ...
                if (subCategory === 'role') {
                    ...
                    if (action === 'add')
                        return handleAdd('role');
                    if (action === 'remove')
                        return handleRemove('role');
                    if (action === 'reset')
                        return handleReset('role');
                    if (action === 'list')
            ...
            async function handleAdd(type) {
                if (!target) {
                    return message.channel.send({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.cross} | Please specify a valid ${type === 'role' ? 'role' : type === 'user' ? 'user' : 'channel'} to add.`)
                        ]
                    });
                }
                let targetObj;
                ...
                if (!targetObj) {
                    return message.channel.send({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.cross} | Please provide a valid ${type === 'role' ? 'role' : type === 'user' ? 'user' : 'channel'}.`)
                        ]
                    });
                }
                ...
                if (data[type].includes(id)) {
                    return message.channel.send({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.cross} | This ${type === 'role' ? 'role' : type === 'user' ? 'user' : 'channel'} is already added.`)
                        ]
                    });
                }
                ...
                if (type !== 'channel' && data[type].length >= 10) {
                    return message.channel.send({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.cross} | You can only add up to 10 ${type === 'role' ? 'roles' : type === 'user' ? 'users' : 'channels'}.`)
                        ]
                    });
                }
                ...
                return message.channel.send({
                    embeds: [
                        client.util.embed()
                            .setColor(client.color)
                            .setDescription(`${client.emoji.tick} | Successfully added the ${type === 'role' ? `role: ${targetObj.name}` : type === 'user' ? `user: ${targetObj.tag || targetObj.user.tag}` : `channel: ${targetObj.name}`}.`)
                    ]
                });
            }
            async function handleRemove(type) {
                if (!target) {
                    return message.channel.send({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.cross} | Please specify a valid ${type === 'role' ? 'role' : type === 'user' ? 'user' : 'channel'} to remove.`)
                        ]
                    });
                }
                let targetObj;
                ...
                if (!data[type].includes(id)) {
                    return message.channel.send({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.cross} | This ${type === 'role' ? 'role' : type === 'user' ? 'user' : 'channel'} is not in the list.`)
                        ]
                    });
                }
                ...
                return message.channel.send({
                    embeds: [
                        client.util.embed()
                            .setColor(client.color)
                            .setDescription(`${client.emoji.tick} | Successfully removed the ${type === 'role' ? `role: ${targetObj.name}` : type === 'user' ? `user: ${targetObj.tag || targetObj.user.tag}` : `channel: ${targetObj.name}`}.`)
                    ]
                });
            }
...
//# sourceMappingURL=mediachannel.js.map