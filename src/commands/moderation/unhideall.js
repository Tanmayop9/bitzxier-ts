    ...
    run: async (client, message, args) => {
        ...
        if (!message.guild.Unhideall) {
            ...
            interactionMessage.edit({
                embeds: [
                    embed.setDescription(`**Are you sure you want to unhide all channels to in this server?**`),
                ], components: [row],
            });
            ...
            collector.on('collect', async (interaction) => {
                if (interaction.isButton()) {
                    if (interaction.customId === 'yes_button') {
                        interactionMessage.edit({
                            embeds: [
                                embed.setDescription(`${client.emoji.tick} | Successfully started unhiding to all channnels.`),
                            ],
                            components: [],
                        });
                        collector.stop();
                        try {
                            message.guild.Unhideall = true;
                            let hided = 0;
                            ...
                            const embed = client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.tick} | Successfully unhidden ${hided} channels`);
                            await message.channel.send({ embeds: [embed] });
                            if (message.guild.Unhideall)
                                message.guild.Unhideall = false;
                        }
                        catch (err) {
                            return;
                        }
                        ...
                        interactionMessage.edit({
                            embeds: [
                                embed.setDescription(`Process canceled.`),
                            ],
                            components: [],
                        });
                        collector.stop();
                ...
                if (collected.size === 0) {
                    interactionMessage.edit({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setTitle('Unhide all Timed Out')
                                .setDescription('The operation of Unhidding channels has been cancelled due to inactivity.')
            ...
            return message.channel.send({
                embeds: [
                    embed.setDescription(`${client.emoji.cross} | There is already a Unhide all process is going on in the server.`)
                        .setColor(client.color),
                ],
            });
        }
...
//# sourceMappingURL=unhideall.js.map