export default {
    ...
    run: async (client, message, args) => {
        ...
        if (!user)
            return message.channel.send({ embeds: [client.util.embed().setColor(client.color).setDescription(`Please provide me a valid user.`)] });
        let reason = args.slice(1).join(' ');
        if (!reason)
            reason = 'No Reason given';
        if (user.id === client.user.id)
            return message.channel.send({ embeds: [client.util.embed().setColor(client.color).setDescription(`Are You Mad ?`)] });
        if (!user.isCommunicationDisabled())
            return message.channel.send({ embeds: [client.util.embed().setColor(client.color).setDescription(`${user} is not muted `)] });
        if (!user.manageable)
            return message.channel.send({ embeds: [client.util.embed().setColor(client.color).setDescription(`I can't unmute that user.Please check my role position and permissions.`)] });
        user.disableCommunicationUntil(null, `${message.author.tag} | ${reason}`);
        return message.channel.send({
            embeds: [
                client.util.embed()
                    .setColor(client.color)
                    .setDescription(`${client.emoji.tick} | Successfully unmuted <@${user.user.id}>!`)
            ]
        });
    }
};
