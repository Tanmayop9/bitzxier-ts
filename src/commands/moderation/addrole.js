    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.permissions.has('ManageRoles')) {
            return message.channel.send({
                embeds: [
                    embed.setDescription(`${client.emoji.cross} | You must have \`Manage Roles\` permissions to use this command.`)
                ]
            });
        }
        if (!message.guild.members.me.permissions.has('ManageRoles')) {
            return message.channel.send({
                embeds: [
                    embed.setDescription(`${client.emoji.cross} | I don't have \`Manage Roles\` permissions to execute this command.`)
                ]
            });
        }
        if (!own && message.member.roles.highest.position <= message.guild.members.me.roles.highest.position) {
            return message.channel.send({
                embeds: [
                    embed.setDescription(`${client.emoji.cross} | You must have a higher role than me to use this command.`)
                ]
            });
        }
        ...
        if (args[0] === 'all') {
            if (!roleToAdd) {
                return message.channel.send({
                    embeds: [
                        client.util.embed().setDescription(`${client.emoji.cross} | You didn't provide a valid role.\n\`${message.guild.prefix}role all <role>\``)
                    ]
                });
            }
            if (roleToAdd.managed) {
                return message.channel.send({
                    embeds: [
                        embed.setColor(client.color).setDescription(`${client.emoji.cross} | This Role Is Managed By Integration`)
                    ]
                });
            }
            if (!roleToAdd.editable) {
                return message.channel.send({
                    embeds: [
                        client.util.embed().setColor(client.color).setDescription(`${client.emoji.cross} | I can't provide this role as my highest role is either below or equal to the provided role.`)
                    ]
                });
            }
            ...
            if (roleToAdd && !message.guild.roleAssignmentActive) {
                ...
                const interactionMessage = await message.channel.send({
                    embeds: [
                        embed.setDescription(`**${client.emoji.process} | Processing Command Please Wait**`),
                    ],
                });
                await client.util.sleep(4000);
                interactionMessage.edit({
                    embeds: [
                        embed.setDescription(`**Are you sure you want to give ${roleToAdd} to everyone in this server?**`),
                    ], components: [row],
                });
                ...
                collector.on('collect', async (interaction) => {
                    if (interaction.isButton()) {
                        if (interaction.customId === 'yes_button') {
                            interactionMessage.edit({
                                embeds: [
                                    embed.setDescription(`${client.emoji.tick} | Successfully started adding <@&${roleToAdd.id}> to all members.`),
                                ],
                                components: [],
                            });
                            collector.stop();
                ...
                return message.channel.send({
                    embeds: [
                        embed.setDescription(`${client.emoji.cross} | There is already a role addition process going on in the server.`)
                            .setColor(client.color),
                    ],
                });
            }
            ...
            if (message.guild.roleAssignmentActive && message.guild.roleToAssign) {
                if (message.guild.roleAssignmentActive)
                    message.guild.roleAssignmentActive = false; // Set roleAssignmentActive to false
                message.guild.roleToAssign = null;
                return message.channel.send({
                    embeds: [
                        embed.setDescription(`${client.emoji.tick} | Successfully Stopped Ongoing Role Addition Process`).setColor(client.color),
                    ],
                });
            }
            else {
                return message.channel.send({
                    embeds: [
                        embed.setDescription(`${client.emoji.cross} | There is no Ongoing Role Addition Process`).setColor(client.color),
                    ],
                });
            }
        }
            ...
            if (message.guild.roleAssignmentActive && message.guild.roleToAssign) {
                ...
                const embed = client.util.embed()
                    ...
                    .addFields({ name: 'Role:', value: role.name || 'Deleted Role' }, { name: 'Percentage of Members:', value: `${percentage.toFixed(2)}%` }, { name: 'Estimated Time:', value: `<t:${etaTimeWilltake}:R>` })
                    .setTimestamp();
                await message.channel.send({ embeds: [embed] });
            }
            else {
            ...
            if (!member) {
                return message.channel.send({
                    embeds: [
                        embed.setColor(client.color).setDescription(`${client.emoji.cross} | You didn't use the command correctly.\n\`${message.guild.prefix}role <user> <role>\``)
                    ]
                });
            }
            ...
            if (!role) {
                return message.channel.send({
                    embeds: [
                        embed.setColor(client.color).setDescription(`${client.emoji.cross} | You didn't provide a valid role.\n\`${message.guild.prefix}role <user> <role>\``)
                    ]
                });
            }
            if (role.managed) {
            ...
            if (!own && message.member.roles.highest.position <= role.position) {
                return message.channel.send({
                    embeds: [
                        embed
                            .setColor(client.color)
                            .setDescription(`${client.emoji.cross} | I can't provide this role as your highest role is either below or equal to the provided role.`)
                    ]
                });
            }
            let hasRole = member.roles.cache.has(role.id);
            if (hasRole) {
                member.roles.remove(role.id, `${message.author.tag}(${message.author.id})`);
                return message.channel.send({
                    embeds: [
                        embed
                            .setColor(client.color)
                ...
                member.roles.add(role.id, `${message.author.tag}(${message.author.id})`);
                return message.channel.send({
                    embeds: [
                        embed
                            .setColor(client.color)
