    ...
    run: async (client, message, args) => {
        ...
        if (!message.guild.members.me.permissions.has('ManageMessages')) {
            return message.channel.send({
                embeds: [
                    client.util.embed()
                        .setColor(client.color)
                        .setDescription(`${client.emoji.cross} | I must have \`Manage Messages\` permissions to use this command.`)
                ]
            });
        }
        if (!args[0]) {
        ...
        if (args[0].toLowerCase() === 'user' || args[0].toLowerCase() === 'users') {
            ...
            if (!args[1]) {
                return message.channel.send({
                    embeds: [
                        client.util.embed()
                            .setColor(client.color)
                            .setDescription(`Please provide valid content message to delete.\nUsage: \`purge user <member> <ammount>\``)
                    ]
                });
            }
            if (!user) {
            ...
            return message.channel.send({
                embeds: [
                    client.util.embed()
                        .setColor(client.color)
                        .setDescription(`${client.emoji.tick} | Successfully deleted ${botmessages.size} messages from ${user}.`)
                ]
            ...
            default:
                ...
                if (isNaN(avg) || avg <= 0) {
                    return message.channel.send({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.cross} | You must provide a valid number of messages to be deleted.`)
                        ]
                    });
                }
                if (avg >= 1000) {
                    return message.channel.send({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.cross} | You can't delete more than **999** messages at a time.`)
                        ]
                    });
                }
                ...
                return message.channel.send({
                    embeds: [
                        client.util.embed()
                            .setColor(client.color)
                            .setDescription(`${client.emoji.tick} | Successfully deleted ${avg} messages.`)
                    ]
                }).then((m) => {
                    setTimeout(() => {
                        m.delete().catch((err) => { });
        ...
        if (messagesToDelete.size === 0) {
            return message.channel.send({
                embeds: [
                    client.util.embed()
                        .setColor(client.color)
                        .setDescription(`There are no messages matching the criteria to delete.`)
                ]
            });
        }
        ...
        return message.channel.send({
            embeds: [
                client.util.embed()
                    .setColor(client.color)
                    .setDescription(`${client.emoji.tick} | Successfully Deleted \`${messagesToDelete.size}\` Messages`)
            ]
        }).then(msg => setTimeout(() => msg.delete(), 4000));
    }
};
