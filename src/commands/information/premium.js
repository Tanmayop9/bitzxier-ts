...
export default {
    ...
    run: async (client, message, args) => {
        ...
        let link = 'https://discord.gg/bitzxier';
        if (!args[0]) {
        ...
        switch (type) {
            case `activate`:
                if (!isprem)
                    return message.reply({
                        embeds: [
                            embed.setDescription(`You don't have any type of premium subscription. Click [here](${link}) to [Purchase](${link}).`)
                        ],
                        components: [row]
                    });
                if (count < 1)
                    return message.reply({
                        embeds: [
                            embed.setDescription(`You have \`0\` Premium Count remaining. Click [here](${link}) to [Purchase](${link}).`)
                        ],
                        components: [row]
                    });
                if (isp === `true`)
                    return message.reply({
                        embeds: [
                            embed.setDescription(`This server's [Premium](${link}) has already been activated by <@${own}>.`)
                        ]
                    });
                if (count > 0) {
            ...
            case `revoke`:
                ...
                if (!isp)
                    return message.reply({
                        embeds: [
                            embed.setDescription(`This server haven't any type of premium subscription! If you are a subscriber, you can **upgrade** this server by typing \`${prefix}premium activate\`\nClick [here](${link}) to [Purchase](${link}).`)
                        ],
                        components: [row]
                    });
                if (own !== message.author.id)
                    return message.reply({
                        embeds: [
                            embed.setDescription(`You haven't activated the [Premium](${link}) on this Server to revoke it.`)
                        ]
                    });
                ...
                return message.reply({
                    embeds: [
                        embed.setDescription(`You have successfully **revoked** the [Premium](${link}) from this server.`)
                    ]
                });
                break;
            case `validity`:
            ...
            case `noprefix`:
                ...
                if (!data.includes(message.author.id)) {
                    ...
                    return message.reply({
                        embeds: [
                            embed.setDescription(`${client.emoji.tick} Your Premium Noprefix Is Sucessfully Activated`)
                        ]
                    });
                }
                else {
                    return message.reply({
                        embeds: [
                            embed.setDescription(`${client.emoji.cross} Your Already Having Noprefix Perk's`)
                        ]
                    });
                }
            default:
