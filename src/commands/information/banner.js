...
export default {
    ...
    run: async (client, message, args) => {
        ...
        const userBanner = userData && userData.banner
            ? `https://cdn.discordapp.com/banners/${member.id}/${userData.banner}${userData.banner.startsWith('a_') ? '.gif' : '.png'}?size=4096`
            : null;
        const serverBanner = guildMember && guildMember.avatar
            ? `https://cdn.discordapp.com/guilds/${message.guild.id}/users/${member.id}/banner/${guildMember.banner}${guildMember.banner.startsWith('a_') ? '.gif' : '.png'}?size=4096`
            : null;
        ...
        const buttons = new ActionRowBuilder().addComponents(new ButtonBuilder()
            ...
            .setDisabled(!userBanner), new ButtonBuilder()
        ...
        collector.on('collect', async (interaction) => {
            if (interaction.user.id !== message.author.id)
                return interaction.reply({ content: 'This is not your interaction..!!', ephemeral: true });
