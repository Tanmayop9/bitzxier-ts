...
class PingCommand extends BaseCommand {
    ...
    async execute(client, message, args) {
        ...
        const getStatus = (ping) => {
            if (ping <= 100)
                return { emoji: '🟢', text: 'Excellent', color: '#00FF00' };
            if (ping <= 200)
                return { emoji: '🟡', text: 'Good', color: '#FFFF00' };
            if (ping <= 400)
                return { emoji: '🟠', text: 'Fair', color: '#FFA500' };
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`### ${status.emoji} Overall Status: ${status.text}\n\n` +
...
//# sourceMappingURL=ping-new.js.map