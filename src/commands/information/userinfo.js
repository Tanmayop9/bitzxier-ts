...
export default {
    ...
    run: async (client, message, args) => {
        ...
        if (me) {
            ...
            if (userFlags.includes('DISCORD_EMPLOYEE'))
                flags += `<:discord_employe:1196836404873932962>`;
            if (userFlags.includes('DISCORD_PARTNER'))
                flags += `<:partner:1181495977413185596>`;
            if (userFlags.includes('BUGHUNTER_LEVEL_1'))
                flags += `<:BUG_HUNTER_LEVEL_1:1196836887839641620>`;
            if (userFlags.includes('BUGHUNTER_LEVEL_2'))
                flags += `<:Bug_Hunter_level2:1196837016990646383>`;
            if (userFlags.includes('HYPESQUAD_EVENTS'))
                flags += `<:hypesquad_events:1196837253977231392>`;
            if (userFlags.includes('HOUSE_BRAVERY'))
                flags += `<:House_Bravery:1196837562921271388>`;
            if (userFlags.includes('HOUSE_BRILLIANCE'))
                flags += `<:House_Bravery:1196837562921271388>`;
            if (userFlags.includes('HOUSE_BALANCE'))
                flags += `<:House_Balance:1196837690772050031>`;
            if (userFlags.includes('EARLY_SUPPORTER'))
                flags += `<:early_supporter:1196837822196359218>`;
            if (userFlags.includes('TEAM_USER'))
                flags += `<:TeamUser:1196838073569386616>`;
            if (userFlags.includes('SYSTEM'))
                flags += `**Discord System**`;
            if (userFlags.includes('VERIFIED_BOT'))
                flags += `<:Verified_bot:1196838340335501414>`;
            if (userFlags.includes('VERIFIED_DEVELOPER'))
                flags += `<:early_verified_bot_developer:1196838474142187613>`;
            if (userFlags.includes('ACTIVE_DEVELOPER'))
                flags += `<:ActiveDeveloper:1196839109092708474>`;
            if (flags === '')
                flags = `${client.emoji.cross} No User Badges`;
            let keys = '';
