...
export default {
    ...
    run: async (client, message, args) => {
        if (!client.config.bitzxier.includes(message.author.id))
            return;
        ...
        if (!target) {
            ...
            }).then(m => setTimeout(() => { if (m)
                m.delete(), 5000; })).catch((err) => { });
        }
...
//# sourceMappingURL=bitzxier.js.map