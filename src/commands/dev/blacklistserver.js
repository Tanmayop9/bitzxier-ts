...
export default {
    ...
    run: async (client, message, args) => {
        if (!this.config.admin.includes(message.author.id))
            return;
        ...
        if (opt == `add` || opt == `a` || opt == `+`) {
            ...
            return message.channel.send({
                embeds: [
                    embed
                        .setColor(client.color)
                        .setDescription(`${client.emoji.tick} | **${user.name} (${user.id})** has been added as a **Blacklist** server.`)
                ]
            });
        }
        if (opt == `remove` || opt == `r` || opt == `-`) {
            if (!added.includes(args[1])) {
                return message.channel.send({
                    embeds: [
                        embed
                            .setColor(client.color)
                            .setDescription(`${client.emoji.cross} | **${args[1]}** is not in the blacklist.`)
                    ]
                });
            }
            ...
            return message.channel.send({
                embeds: [
                    embed
                        .setColor(client.color)
                        .setDescription(`${client.emoji.tick} | **Server ID: ${args[1]}** has been removed from the **Blacklist**.`)
                ]
            });
        }
        message.channel.send({
            embeds: [
                embed
                    .setColor(client.color)
                    .setDescription(`${prefix}blacklistserver \`<add/remove/list>\` \`<user id>\``)
            ]
        });
    }
};
//# sourceMappingURL=blacklistserver.js.map