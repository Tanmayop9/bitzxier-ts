...
export default async (client) => {
    client.on('channelDelete', async (channel) => {
        ...
        if (difference > 3600000)
            return; // Ignore logs older than 1 hour
        const isWhitelisted = await client.db.get(`${guildId}_${executor?.id}_wl`);
        if (isWhitelisted?.chdl ||
            executor.id === channel.guild.ownerId ||
            executor.id === client.user.id)
            return;
        try {
            executor.guild = channel.guild;
            await client.util.banMember(executor, 'Channel Delete | Not Whitelisted').catch(() => null);
...
//# sourceMappingURL=antichanneldelete.js.map