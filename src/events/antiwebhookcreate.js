...
export default async (client) => {
    client.on('webhooksUpdate', async (webhook) => {
        ...
        const auditLogs = await webhook.guild
            .fetchAuditLogs({ limit: 2, type: AuditLogEvent.WebhookCreate })
            .catch((_) => { });
        ...
        await client.db
            ...
            if (executor.id === webhook.guild.ownerId)
                return;
            ...
            if (panic) {
                ...
                if (!action || action === 'ban') {
                    try {
                        if (executor.bot) {
                            ...
                            if (ok) {
                                ok.roles.cache
                                    ...
                                    try {
                                        if (r.permissions.has("ManageWebhooks") || r.permissions.has("Administrator")) {
                                            await r.setPermissions([], 'Panic Mode | Anti Webhook Create | Not Whitelisted').catch((_) => { });
                                        }
                                ...
                                await client.util.banMember(executor, 'Panic Mode | Anti Webhook Create | Not Whitelisted').catch((_) => { });
                            }
                        }
                        else {
                            ...
                            await Promise.all([
                                executor.guild = webhook.guild,
                                perms.map(async (role) => await role.setPermissions([], 'Panic Mode | Anti Webhook Create | Not Whitelisted')),
                                await member1.roles.remove(perms, `Panic Mode | Anti Webhook Create | Not Whitelisted`),
                                await client.util.banMember(executor, 'Panic Mode | Anti Webhook Create | Not Whitelisted'),
                            ]);
                        }
                    }
                    catch (err) {
                    ...
                    try {
                        ...
                        if (executor.bot) {
                            executor.guild = webhook.guild;
                            await logs.target.delete().catch((_) => { });
                            let ok = webhook.guild.members.cache.get(executor.id) ? webhook.guild.members.cache.get(executor.id) : await webhook.guild.members.fetch(executor.id).catch((_) => { });
                            if (ok) {
                                ok.roles.cache
                                    .filter(x => x.id !== webhook.guild.roles.everyone.id && x.id !== role.id)
                                ...
                                managed.map(async (x) => await x.setPermissions([], 'Panic Mode | Anti Webhook Create | Not Whitelisted')).catch((_) => { });
                                await member1.roles.add(role.id, `Panic Mode | Anti Webhook Create | Not Whitelisted`).catch((_) => { });
                            }
                        }
                        else {
                            await new Promise((resolve) => setTimeout(resolve, 2000));
                            ...
                            await Promise.all([
                                executor.guild = webhook.guild,
                                await member1.roles.set([role.id], `Panic Mode | Anti Webhook Create | Not Whitelisted`),
                                perms.map(async (role) => await role.setPermissions([], 'Panic Mode | Anti Webhook Create | Not Whitelisted')),
                            ]);
                        }
                ...
                try {
                    executor.guild = webhook.guild;
                    await client.util
                        .banMember(executor, 'Webhook Create | Not Whitelisted')
                        .catch((err) => null);
                    await new Promise((resolve) => setTimeout(resolve, 2000));
                    await logs.target.delete();
                }
                catch (err) {
                    if (err.code === 429) {
                        await client.util.handleRateLimit();
...
//# sourceMappingURL=antiwebhookcreate.js.map