export default async (client) => {
    ...
    client.on('messageCreate', async (message) => {
        ...
        if (message.webhookId)
            return;
        if (await client.util.BlacklistCheck(message?.guild))
            return;
        try {
            ...
            if (isUserWhitelisted || isRoleWhitelisted || isChannelWhitelisted)
                return;
            let punishment = (await client.db.get(`antilinkp_${message.guild.id}`)) ?? { data: null };
                ...
                try {
                    if (!action || action === 'mute') {
                        ...
                        if (message.member.manageable) {
                            if (message.member.isCommunicationDisabled())
                                return;
                            try {
                                await message.member.timeout(30 * 60 * 1000, 'BITZXIER | ANTILINK | TIMEOUT');
                                let scucess = await message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                                            .setDescription(`${message.author.tag}  has been muted for sending links.`)
                                    ]
                                });
                    ...
                    if (action === 'kick') {
                        ...
                        if (message.member.kickable) {
                            try {
                                await message.guild.members.kick(message.member.id, 'BITZXIER | ANTILINK | KICK');
                                let scucess = await message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                                            .setDescription(`${message.author.tag} has been kicked for sending links.`)
                                    ]
                                });
                                await client.util.sleep(3000);
                                await scucess.delete();
                            ...
                            try {
                                ...
                                let scucess = await message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                                            .setDescription(`${message.author.tag} has been banned for sending links.`)
                                    ]
                                });
                                await client.util.sleep(3000);
                                await scucess.delete();
    ...
    client.on('messageUpdate', async (oldMessage, newMessage) => {
        ...
        if (oldContent === newContent)
            return; // No change in message, no need to proceed
        if (await client.util.BlacklistCheck(newMessage?.guild))
            return;
            ...
            try {
                if (!action || action === 'mute') {
                    ...
                    if (newMessage.member.manageable) {
                        if (newMessage.member.isCommunicationDisabled())
                            return;
                        try {
                ...
                if (userData.warnings >= warns) {
                        ...
                        try {
                            if (action === 'mute' || !action) {
                                if (member.manageable && !member.isCommunicationDisabled()) {
                                    await member.timeout(30 * 60 * 1000, 'BITZXIER | ANTISPAM | TIMEOUT');
                                ...
                                let successMessage = await message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                                            .setDescription(`${message.author.tag} has been kicked for spamming.`)
                                    ]
                                });
                                setTimeout(() => successMessage.delete(), 3000);
                            }
                                ...
                                let successMessage = await message.channel.send({
                                    embeds: [
                                        client.util.embed()
                                            .setColor(client.color)
                                            .setDescription(`${message.author.tag} has been banned for spamming.`)
                                    ]
                                });
                                setTimeout(() => successMessage.delete(), 3000);
                            }
                    ...
                    let successMessage = await message.channel.send({
                        embeds: [
                            client.util.embed()
                                .setColor(client.color)
                                .setDescription(`${message.author}, this is your warning ${userData.warnings}/${warns} for spamming.`)
                        ]
                    });
                    await deleteMessages(userId, userData.messageIds);
                    setTimeout(() => successMessage.delete(), 3000);
    ...
    client.on('messageCreate', async (message) => {
        ...
        try {
            ...
            if (antiswear !== true)
                return;
            const userId = message.author.id;
            if (message.author.id === client.user.id || message.author.id === message.guild.ownerId || !message.guild || !message.member || message.member.permissions.has('Administrator'))
                return;
            const whitelistData = await client.db.get(`automodbypass_${message?.guild?.id}`) || { user: [], role: [], channel: [] };
            const { user: userWhitelist = [], role: roleWhitelist = [], channel: channelWhitelist = [] } = whitelistData;
            const isUserWhitelisted = userWhitelist.some(userEntry => {
            ...
            if (isUserWhitelisted || isRoleWhitelisted || isChannelWhitelisted)
                return;
            let punishment = (await client.db.get(`antiswearp_${message.guild.id}`)) || { data: null };
                ...
                if (!action || action === 'mute') {
                    ...
                    if (message.member.manageable) {
                        if (message.member.isCommunicationDisabled())
                            return;
                        try {
                            await message.member.timeout(30 * 60 * 1000, 'BITZXIER | ANTI WORDS | TIMEOUT');
                            let success = await message.channel.send({
                                embeds: [
                                    client.util.embed()
                                        .setColor(client.color)
                                        .setDescription(`${message.author.tag} has been muted for using badwords.`)
                                ]
                            });
                            await client.util.sleep(3000);
                            await success.delete();
                    ...
                    if (message.member.kickable) {
                        try {
                            await message.member.kick('BITZXIER | ANTI WORDS | KICK');
                            let success = await message.channel.send({
                                embeds: [
                                    client.util.embed()
                                        .setColor(client.color)
                                        .setDescription(`${message.author.tag} has been kicked for using badwords.`)
                                ]
                            });
                            await client.util.sleep(3000);
                            await success.delete();
                    ...
                    if (message.member.banable) {
                        try {
                            await message.member.ban('BITZXIER | ANTI WORDS | BAN');
                            let success = await message.channel.send({
                                embeds: [
                                    client.util.embed()
                                        .setColor(client.color)
                                        .setDescription(`${message.author.tag} has been banned for using badwords.`)
                                ]
                            });
                            await client.util.sleep(3000);
                            await success.delete();
    ...
    client.on('messageUpdate', async (oldMessage, newMessage) => {
        ...
        try {
            ...
            if (antiswear !== true)
                return;
            const userId = newMessage.author.id;
            if (newMessage.author.id === client.user.id || newMessage.author.id === newMessage.guild.ownerId || !newMessage.guild || !newMessage.member || newMessage.member.permissions.has('Administrator'))
                return;
            const whitelistData = await client.db.get(`automodbypass_${newMessage?.guild?.id}`) || { user: [], role: [], channel: [] };
            const { user: userWhitelist = [], role: roleWhitelist = [], channel: channelWhitelist = [] } = whitelistData;
 ...
 */
    // ANTI INVITE
    client.on('messageCreate', async (message) => {
        if (message.author.bot || message.author.id === client.user.id || message.author.id === message.guild.ownerId || !message.guild || !message.member || message.member.permissions.has('Administrator') || message.webhookId)
            return;
        if (await client.util.BlacklistCheck(message?.guild))
            return;
        try {
            let antiinvite = await client.db.get(`antiinvite_${message?.guild?.id}`);
            if (antiinvite.enabled !== true)
                return;
            const userId = message.author.id;
            const whitelistData = await client.db.get(`automodbypass_${message?.guild?.id}`) || { user: [], role: [], channel: [] };
            const { user: userWhitelist = [], role: roleWhitelist = [], channel: channelWhitelist = [] } = whitelistData;
            ...
            if (urls && urls.some((url) => !isAllowedLink(url))) {
                ...
                if (!action || action === "mute") {
                    ...
                    try {
                        if (!client.recentlyinvitelinkwarned)
                            client.recentlyinvitelinkwarned = new Map();
                        if (!client.recentlyinvitelinkwarned.has(message.guild.id))
                            client.recentlyinvitelinkwarned.set(message.guild.id, new Set());
            ...
            if (urls && urls.some((url) => !isAllowedLink(url))) {
                ...
                if (!action || action === "mute") {
                    ...
                    try {
                        if (!client.recentlyinvitelinkwarned)
                            client.recentlyinvitelinkwarned = new Map();
                        if (!client.recentlyinvitelinkwarned.has(newMessage.guild.id))
                            client.recentlyinvitelinkwarned.set(newMessage.guild.id, new Set());
