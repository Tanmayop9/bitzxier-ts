...
export default async (client) => {
    ...
    client.on('messageCreate', async (message) => {
        ...
        if (check.includes(message?.guild?.id))
            return;
        const settings = await getSettings(message?.guild);
        if (!settings)
            return;
        if (!settings.mainrole)
            return;
        const checkExist = settings.mainrole.filter((_role) => message.mentions.roles.map((r) => r.id).includes(_role));
        if (checkExist.length > 0) {
            await client.db
                ...
                if (panic) {
                    ...
                    try {
                        message.author.guild = message.guild;
                        await client.util.banMember(message.author, 'Mentioned Mainrole | Not Whitelisted').catch((err) => null);
                        const permissions = message.channel.permissionsFor(message.guild.roles.everyone);
                        if (!permissions || !permissions.has(PermissionsBitField.Flags.ViewChannel)) {
                            await message.delete().catch((_) => { });
                        }
...
//# sourceMappingURL=antiroleping.js.map