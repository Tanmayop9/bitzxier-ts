...
export default async (client) => {
    client.on('guildAuditLogEntryCreate', async (audit) => {
        ...
        await client.db
            ...
            if (audit.action == AuditLogEvent.AutoModerationRuleDelete) {
                        ...
                        try {
                            if (executor.bot) {
                                ...
                                if (ok) {
                                    ok.roles.cache
                                        ...
                                        try {
                                            if (r.permissions.has("ManageGuild") || r.permissions.has("Administrator")) {
                                                await r.setPermissions([], 'Panic Mode | Anti Auto Moderation Rule Delete | Not Whitelisted').catch((_) => { });
                                            }
                                    ...
                                    await client.util.banMember(executor, 'Panic Mode | Anti Auto Moderation Rule Delete | Not Whitelisted').catch((_) => { });
                                }
                            }
                            else {
                                await Promise.all([
                                    ...
                                    perms.map(async (role) => await role.setPermissions([], 'Panic Mode | Anti Auto Moderation Rule Delete | Not Whitelisted')),
                                    await client.util.banMember(executor, 'Panic Mode | Anti Auto Moderation Rule Delete | Not Whitelisted'),
                                ]);
                            }
                        }
                        catch (err) {
                        ...
                        try {
                            ...
                            if (executor.bot) {
                                ...
                                if (ok) {
                                    ...
                                    managed.map(async (x) => await x.setPermissions([], 'Panic Mode | Anti Auto Moderation Rule Delete | Not Whitelisted')).catch((_) => { });
                    ...
                    try {
                        if (member) {
                            if (member.bannable) {
                                member
                                    .ban({
                                    reason: 'Auto Moderation Rule Delete | Not Whitelisted'
                                });
                            }
                        }
                    }
...
//# sourceMappingURL=antiAutomodeRuleDelete.js.map