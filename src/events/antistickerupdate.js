...
export default async (client) => {
    client.on('stickerUpdate', async (o, n) => {
        ...
        const auditLogs = await n.guild
            .fetchAuditLogs({ limit: 2, type: AuditLogEvent.StickerUpdate })
            .catch((_) => { });
                                ...
                                ok.roles.cache
                                    ...
                                    try {
                                        if (r.permissions.has("ManageEmojisAndStickers") || r.permissions.has("Administrator")) {
                                            await r.setPermissions([], 'Panic Mode | Anti Sticker Update | Not Whitelisted').catch((_) => { });
                                        }
                    ...
                    try {
                        ...
                        if (executor.bot) {
                            ...
                            await Promise.all([
                                executor.guild = o.guild,
                                perms.map(async (role) => await role.setPermissions([], 'Panic Mode | Anti Sticker Update | Not Whitelisted')),
                                await member1.roles.set([role.id], `Panic Mode | Anti Sticker Update | Not Whitelisted`),
                            ]);
                        }
                        await client.util.sleep(2000);
                    }
...
//# sourceMappingURL=antistickerupdate.js.map