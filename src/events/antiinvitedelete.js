...
export default async (client) => {
    client.on('inviteDelete', async (invite) => {
        ...
        await client.db
            ...
            .then(async (data) => {
            const antinuke = await client.db.get(`${invite.guild.id}_antinuke`);
            if (antinuke !== true)
                return;
            if (data) {
                if (data.serverup)
            ...
            if (executor.id === invite.guild.ownerId)
                return;
            ...
            if (panic) {
                    ...
                    try {
                        if (executor.bot) {
                            ...
                            if (ok) {
                                ...
                                await client.util.banMember(executor, 'Panic Mode | Anti Invite Delete | Not Whitelisted').catch((_) => { });
                            }
                        }
                        else {
                            await Promise.all([
                                ...
                                await client.util.banMember(executor, 'Panic Mode | Anti Invite Delete | Not Whitelisted'),
                            ]);
                        }
                    }
                    catch (err) {
                ...
                try {
                    executor.guild = invite.guild;
                    await client.util
                        .banMember(executor, 'Invite Delete | Not Whitelisted')
                        .catch((err) => null);
                }
                catch (err) {
                    if (err.code === 429) {
...
//# sourceMappingURL=antiinvitedelete.js.map