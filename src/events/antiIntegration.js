...
export default async (client) => {
    client.on('guildAuditLogEntryCreate', async (audit, guild) => {
        try {
            if (audit.action != AuditLogEvent.IntegrationCreate)
                return;
            const check = await client.db.get(`blacklistserver_${client.user.id}`) || [];
            if (check.includes(guild.id))
                return;
            ...
            if (isWhitelisted?.botadd || executorId === guild.ownerId || executorId === client.user.id)
                return;
            try {
...
//# sourceMappingURL=antiIntegration.js.map