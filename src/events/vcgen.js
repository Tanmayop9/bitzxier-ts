import { ActionRowBuilder, StringSelectMenuBuilder, ModalBuilder, TextInputBuilder, PermissionFlagsBits, ChannelType } from 'discord.js';
            ...
            if (tempChannel) {
                ...
                if (oldState.member.id === tempChannel.ownerId) {
                    if (voiceChannel.members.size > 0 && !newState.member.user.bot) {
                        ...
                        if (newOwner) {
                            await GuildConfig.updateOne({ guildId: guild.id, 'tempVoiceChannels.channelId': voiceChannel.id }, { $set: { 'tempVoiceChannels.$.ownerId': newOwner.id } });
                            await voiceChannel.setName(`${newOwner.user.displayName}'s Vc`);
                        }
                    }
                    else {
                        await voiceChannel.delete();
                        await GuildConfig.updateOne({ guildId: guild.id }, { $pull: { tempVoiceChannels: { channelId: voiceChannel.id } } });
                    }
                }
                ...
                switch (interaction.customId) {
                    ...
                    case 'vcmute':
                        ...
                        const muteRow = new ActionRowBuilder()
                            ...
                            .addOptions(voiceChannel.members
                    ...
                    case 'selectUserToTransferOwnership':
                        ...
                        if (newOwner) {
                            await GuildConfig.updateOne({ guildId: interaction.guild.id, 'tempVoiceChannels.channelId': voiceChannel.id }, { $set: { 'tempVoiceChannels.$.ownerId': newOwner.id } });
                            await voiceChannel.setName(`${newOwner.user.displayName}'s Vc`);
