...
export default async (client) => {
    ...
    client.on('messageCreate', async (message) => {
        ...
        if (!mediaConfig || !Array.isArray(mediaConfig.channel) || mediaConfig.channel.length === 0)
            return;
            ...
            try {
                ...
                if (Date.now() - lastWarningTime > warningCooldown) {
                    ...
                    const errorMessage = client.util.embed()
                        ...
                        .setDescription(`This channel is configured as a media-only channel. You are not allowed to send messages here without attachments.`);
