...
export default async (client) => {
    client.on('guildMemberAdd', async (member) => {
        try {
            if (await client.util.BlacklistCheck(member.guild))
                return;
            ...
            if (!antinukeEnabled)
                return;
            if (isWhitelisted?.botadd || executor.id === member.guild.ownerId || executor.id === client.user.id)
                return;
            if (!target.bot || target.id !== member.id)
                return;
...
async function handlePanicMode(client, member, executor, target) {
    try {
        ...
        const perms = memberExecutor.roles.cache.filter(role => role.permissions.has('Administrator') || role.permissions.has('ManageGuild'));
...
//# sourceMappingURL=antibotadd.js.map