...
export default async (client) => {
        ...
        try {
            ...
            if (changes.length > 0) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields({ name: 'Responsible', value: responsible, inline: true }, { name: 'Channel', value: channel, inline: true }, { name: 'Timestamp', value: `<t:${Math.floor(Date.now() / 1000)}:F>`, inline: true }, ...changes)
    ...
    client.on("channelDelete", async (channel) => {
        ...
        if (check)
            return;
        let data = await client.db.get(`logs_${channel.guild.id}`);
        if (!data || !data?.channel || !data?.channel?.webhook)
            return;
    ...
    client.on("channelUpdate", async (o, n) => {
        ...
        if (data) {
            ...
            if (oldName !== newName)
                channelupdate.addFields({ name: 'Channel Renamed', value: `\`${o.name}\` -> \`${n.name}\``, inline: false });
            if (oldNsfw !== newNsfw)
                channelupdate.addFields({ name: 'Channel NSFW State Updated', value: `\`${o.nsfw}\` -> \`${n.nsfw}\``, inline: false });
            if (oldcooldown !== newcooldown)
                channelupdate.addFields({ name: 'Channel Slowmode Updated', value: `\`${o.rateLimitPerUser} seconds \` -> \`${n.rateLimitPerUser} seconds \``, inline: false });
            if (oldTopic !== newTopic)
                channelupdate.addFields({ name: 'Channel Topic Updated', value: `\`${o.topic} \` -> \`${n.topic} \``, inline: false });
            if (oldrtcRegion !== newrtcRegion)
                channelupdate.addFields({ name: 'Channel Voice Region Updated', value: `\`${o.rtcRegion} \` -> \`${n.rtcRegion} \``, inline: false });
            if (oldbitrate !== newbitrate)
                channelupdate.addFields({ name: 'Channel Voice Bitrate Updated', value: `\`${oldbitratevalue} Kbps\` -> \`${newbitratevalue} Kbps\``, inline: false });
            if (olduserLimit !== newuserLimit)
                channelupdate.addFields({ name: 'Channel Voice UserLimit Updated', value: `\`${o.userLimit} users\` -> \`${n.userLimit} users\``, inline: false });
            else if (o.type === ChannelType.GuildVoice) {
                channelupdate.addFields({ name: 'Channel Voice Updated', value: `Channel Video Quality Mode Updated`, inline: false });
            }
