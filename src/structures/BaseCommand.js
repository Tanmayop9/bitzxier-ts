...
class BaseCommand {
    ...
    async run(client, message, args) {
        ...
        if (!userPerms.success) {
            return message.channel.send({
                embeds: [this.createErrorEmbed(client, 'Missing Permissions', `You need the following permissions: ${userPerms.missing.join(', ')}`)]
            });
        }
        ...
        if (!botPerms.success) {
            return message.channel.send({
                embeds: [this.createErrorEmbed(client, 'Missing Bot Permissions', `I need the following permissions: ${botPerms.missing.join(', ')}`)]
            });
        }
        // Execute command
        try {
            ...
            return message.channel.send({
                embeds: [this.createErrorEmbed(client, 'Command Error', 'An error occurred while executing this command')]
            });
        }
    }
}
