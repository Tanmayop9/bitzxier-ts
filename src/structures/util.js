import { EmbedBuilder, Collection, ButtonStyle, ActionRowBuilder, ButtonBuilder, PermissionsBitField, ChannelType } from 'discord.js';
...
export default class Util {
    client;
    blacklistCache;
    lastBlacklistRefresh;
    BLACKLIST_CACHE_DURATION;
    constructor(client) {
        this.client = client;
    ...
    async setChannel(settings, channel) {
        if (!this.client.util.canSendEmbeds(channel)) {
            return ('Ugh! I cannot send greeting to that channel? I need the `Write Messages` and `Embed Links` permissions in ' +
                channel.toString());
        }
            ...
            return {
                ...
                embeds: [
                    this.client.util.embed()
                        ...
                        .setDescription(`Hey ${member.displayName}, Welcome to the server <a:welcome:1188456678392348702>.`)
                ]
            };
        }
    ...
    async isExtraOwner(member, guild) {
        ...
        if (!data)
            return false;
        if (data?.owner?.includes(member.id))
            return true;
        else
            return false;
    }
        ...
        try {
            ...
            if (funny === 'antinuke') {
                ...
                if (joinedCmdList.length <= 1024) {
                    embed1.addFields({ name: `**<:bitzxier_antinuke:1181289584483643433> Antinuke \`[${cmdList.length}]\`**`, value: joinedCmdList });
                    ...
                    embed1.addFields({ name: `**<:bitzxier_antinuke:1181289584483643433> Antinuke \`[${half}]\`**`, value: firstHalf });
                    embed2.addFields({ name: `**<:bitzxier_antinuke:1181289584483643433> Antinuke \`[${cmdList.length - half}]\`**`, value: secondHalf });
            ...
            if (funny === 'moderation') {
                ...
                const joinedCmdList = cmdList.sort().join(', ');
                if (joinedCmdList.length <= 1024) {
                    embed1.addFields({ name: `**<:bitzxier_moderator:1181290384576491561> Moderation \`[${cmdList.length}]\`**`, value: joinedCmdList });
                    interaction.reply({ embeds: [embed1], ephemeral: true });
                }
                else {
                    const half = Math.ceil(cmdList.length / 2);
                    const firstHalf = cmdList.slice(0, half).join(', ');
                    const secondHalf = cmdList.slice(half).join(', ');
                    embed1.addFields({ name: `**<:bitzxier_moderator:1181290384576491561> Moderation \`[${half}]\`**`, value: firstHalf });
                    embed2.addFields({ name: `**<:bitzxier_moderator:1181290384576491561> Moderation \`[${cmdList.length - half}]\`**`, value: secondHalf });
                    interaction.reply({ embeds: [embed1, embed2], ephemeral: true });
                }
                return;
            }
            if (funny === 'automod') {
                ...
                embed.addFields({
                    name: `**<:Bitzxier_Automod:1205791245473943553> Automod \`[${cmdList.length}]\`**`,
            ...
            if (funny === 'utility') {
                ...
                embed.addFields({
                    name: `**<:bitzxier_utility:1181291761667149886> Utility  \`[${cmdList.length}]\`**`,
                    value: cmdList.sort().join(', ')
                });
                interaction
                    .reply({
            ...
            if (funny === 'jointocreate') {
                ...
                embed.addFields({
                    name: `**<:bitzxierjtc:1287754917942923347> Join To Create \`[${cmdList.length}]\`**`,
                    value: cmdList.sort().join(', ')
                });
                await interaction
                    .reply({
            ...
            if (funny === 'voice') {
                ...
                embed.addFields({
                    name: `**<:bitzxier_mic:1181294198046072994> Voice \`[${cmdList.length}]\`**`,
                    value: cmdList.sort().join(', ')
                });
                await interaction
                    .reply({
            ...
            if (funny === 'welcomer') {
                ...
                embed.addFields({
                    name: `**<:bitzxier_autorole:1181290290238210158> Welcomer \`[${cmdList.length}]\`**`,
                    value: cmdList.sort().join(', ')
                });
                await interaction
                    .reply({
            ...
            if (funny === 'ticket') {
                ...
                const joinedCmdList = cmdList.sort().join(', ');
                if (joinedCmdList.length <= 1024) {
                    embed1.addFields({ name: `**<:bitzxierticket:1287755212898959472> Ticket \`[${cmdList.length}]\`**`, value: joinedCmdList });
                    interaction.reply({ embeds: [embed1], ephemeral: true });
                }
                else {
                    const half = Math.ceil(cmdList.length / 2);
                    const firstHalf = cmdList.slice(0, half).join(', ');
                    const secondHalf = cmdList.slice(half).join(', ');
                    embed1.addFields({ name: `**<:bitzxierticket:1287755212898959472> Ticket \`[${half}]\`**`, value: firstHalf });
                    embed2.addFields({ name: `**<:bitzxierticket:1287755212898959472> Ticket \`[${cmdList.length - half}]\`**`, value: secondHalf });
                    interaction.reply({ embeds: [embed1, embed2], ephemeral: true });
                }
                return;
            }
            ...
            if (data) {
                message.reply({
                    embeds: [
                        client.util.embed()
                            .setColor(client.color)
                            .setDescription(`<@${memberMentioned.id}> went AFK <t:${Math.round(data.Time / 1000)}:R>\n\nFor Reason: **${data.Reason}**`)
                    ]
                });
            }
            else {
    ...
    async banMember(member, reason = 'Not Whitelisted | Performed Suspicious Activity') {
        try {
    ...
    async sendBooster(guild, member) {
        ...
        try {
            ...
            const embed = this.client.util.embed()
                ...
                .setDescription(`**<@${member.id}> Just Boosted ${guild.name}. Thank You So Much For Boosting Our Server. We Now Have Total ${count} Boosts On Our Server!!**`)
                .setFooter({
                text: `Server Boosted 🎉 `,
